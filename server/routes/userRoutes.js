...
const registerUser = asyncHandler(async (req, res) => {
	...
	if (userExists) {
		res.status(400).send("We already have an account with that email adress");
	}
	...
	if (user) {
		...
		throw new Error("Something went worng. Please check your information and try again.");
	}
});
...
const passwordReset = asyncHandler(async (req, res) => {
	...
	try {
		...
		if (user) {
			...
			await user.save();
			res.json("Your password has been update successfully.");
		} else {
			res.status(404).send("User not found.");
		}
	} catch (error) {
...
const googleLogin = asyncHandler(async (req, res) => {
	...
	try {
		...
		res.status(404).send("Something went wrong, pelase try again later.");
	}
});
